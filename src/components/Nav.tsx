...
export function Nav(props: { html: string }) {
    ...
    useEffect(() => {
        ...
        const links = Array.from(el.querySelectorAll('.nav_right a')) as HTMLElement[];

        [c, city, h, m, a].forEach((node) => node && bus.dispatch({ state: 0, style: 0, el: node }));
        el.style.opacity = '1';
        [c, city, h, m, a].forEach((node) => node && bus.dispatch({ state: 1, style: 0, el: node }));
        links.forEach((lnk) => {
