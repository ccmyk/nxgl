...
export function TtF({
	...
	useEffect(() => {
		...
		const handleMouseEnter = () => {
			interactionState.current.lerp = 0.02;
		};
		const handleMouseMove = (e: MouseEvent) => {
			const bounds = interactionState.current.bounds;
			const normY = (e.clientY - bounds.top) / bounds.height;
			interactionState.current.norm = clamp(0, 1, normY);
		};
		const handleMouseLeave = () => {
			interactionState.current.lerp = 0.01;
		};
