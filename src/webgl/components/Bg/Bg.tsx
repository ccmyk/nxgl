...
export function Bg({ id, scrollTarget, onStateChange, behind }: Props) {
  ...
  useEffect(() => {
    ...
    anims.current.scroll = gsap.timeline({ paused: true })
      ...
      .fromTo('.nav', 
        ...
        { 
          '--dark': '#000', '--light': '#F8F6F2', duration: 0.5,
    ...
    const handleResize = () => {
        ...
        if (scrollTargetRef.current) {
            ...
            scrollState.current.start = rect.top + window.scrollY - window.innerHeight;
  ...
  }, [id, behind, scrollTargetRef, onStateChange, register, unregister, isActive]);
