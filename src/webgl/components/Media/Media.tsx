...
export function Media({ id, holder, src, invert, isTouch }: Props) {
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ alpha: true, dpr: Math.max(window.devicePixelRatio, 2), width: holder.clientWidth, height: holder.clientHeight });
    ...
    anims.current.scroll = gsap.timeline({ paused: true })
      .fromTo(program.uniforms.uStart, { value: invert ? -0.8 : 0.8 }, {
        value: 0,
        ease: 'power2.inOut',
        onComplete: () => {
    ...
    register({
      ...
      onTick: (time) => {
        ...
        if (media instanceof HTMLVideoElement && media.readyState >= media.HAVE_ENOUGH_DATA) {
          texture.needsUpdate = true;
          program.uniforms.uTextureSize.value = [media.videoWidth, media.videoHeight];
        } else if (media instanceof HTMLImageElement) {
          program.uniforms.uTextureSize.value = [media.naturalWidth, media.naturalHeight];
        }
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent | TouchEvent) => {
      ...
      interactionState.current.norm[0] = (clientX - bounds.left) / bounds.width - 0.5;
    };
