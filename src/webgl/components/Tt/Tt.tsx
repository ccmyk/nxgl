...
import { useFrame, useOGL } from "@/webgl/hooks/useGL";
...
export function Tt({
	...
	const { gl } = useOGL();
	const { png, json } = useMSDF(
		"/fonts/msdf/PPNeueMontreal-Medium.png",
		"/fonts/msdf/PPNeueMontreal-Medium.json",
	);
	...
	useEffect(() => {
		...
		anims.current.in = gsap
			.timeline({ paused: true })
			.to(program.uniforms.uPower, {
				value: 1,
				duration: 0.36,
				ease: "power4.inOut",
			});
		anims.current.out = gsap
			.timeline({ paused: true })
			.to(program.uniforms.uPower, {
				value: 0,
				duration: 0.6,
				ease: "none",
				onComplete: () => {
					program.uniforms.uKey.value = -1;
				},
			});
		anims.current.start = gsap
			.timeline({ paused: true })
			.fromTo(
			...
			.fromTo(
				program.uniforms.uPower,
				{ value: 0.5 },
				{ value: 0, duration: 2, ease: "power2.inOut" },
				0,
			)
			.set(
				program.uniforms.uKey,
				{
					value: -1,
					onComplete: () => {
	...
	useEffect(() => {
		...
		const calcChars = (x: number, out?: number) => {
			if (out !== undefined) {
				interactionState.current.positionTarget = new Array(
					st.chars.length,
				).fill(out);
			} else {
				interactionState.current.positionTarget = st.chars.map((_, i) => {
					...
					const tot = (x - charPos) / charWidth - 0.5;
		...
		const handleCharHover = (e: Event) => {
			...
			if (programRef.current)
				programRef.current.uniforms.uKey.value = charIndex;
		};
		...
		return () => {
			...
			st.chars.forEach((char) =>
				char.removeEventListener("mouseenter", handleCharHover),
			);
			st.revert();
		};
	}, [isReady, overlay]);
